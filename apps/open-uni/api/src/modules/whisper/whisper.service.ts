...
export class WhisperService {
  async transcribe(audioPath: string, apiKey: string, onProgress = (_: string) => {}): Promise<{ text: string; segments: any[] }> {
    onProgress('מתמלל עם Groq Whisper API...');
    console.log('🎙️   Transcribing with groq-whisper...');
    const result = await transcribeGroq(audioPath, apiKey);
